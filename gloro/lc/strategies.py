...
from gloro.utils import l2_normalize, linf_normalize
...
class LipschitzComputationStrategy(object):
    ...
    def get(strategy):
        ...
        elif strategy == "power_inf":
            return PowerMethodInf()
    ...
    def for_layer_inf(layer):
        ...
        if isinstance(layer, KerasDense):
            return PowerMethodInf().build(layer.kernel)

        elif isinstance(layer, KerasConv2D):
            return PowerMethodInf().build(
                layer.kernel,
                input_shape=layer.input_shape,
                strides=layer.strides,
                padding=layer.padding.upper(),
...
class PowerMethodInf(LipschitzComputationStrategy):
    def __init__(self, iterations=10, convergence_threshold=1e-5):
        self._iterations = tf.Variable(
            iterations,
            dtype="int32",
    ...
    def build(self, W, input_shape=None, strides=None, padding=None, **kwargs):
        if len(W.shape) == 2:
            ...
            self._forward = lambda x: W @ x
            self._transpose = lambda Wx: tf.transpose(W) @ Wx
            self._normalize = lambda x: linf_normalize(x, axis=0)
            ...
            self._normalize = lambda x: linf_normalize(x, axis=(1, 2, 3))
    ...
    def compute(self):
        def body(i, x, diff):
            ...
            diff = tf.reduce_max(tf.abs(x - x_orig))
        ...
        return tf.reduce_max(tf.abs(self._forward(x))) / (
            tf.reduce_max(tf.abs(x)) + gloro.constants.EPS
        )
